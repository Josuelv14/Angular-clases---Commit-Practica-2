...
import { Firestore, collection, query, where, collectionData, doc, setDoc, deleteDoc, getDoc } from '@angular/fire/firestore';
...
export class FavoritesService {
  ...
  // Comprueba si el usuario ya guardó este personaje.
  async isFavorite(userId: string, characterId: number): Promise<boolean> {
    const ref = doc(this.firestore, `favorites/${userId}-${characterId}`);
